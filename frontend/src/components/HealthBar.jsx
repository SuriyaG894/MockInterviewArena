export default function HealthBar({ label, hp, isPlayer }) {
  ...
  let labelColor = 'text-cyan-600 dark:text-cyan-400';
  if (clampedHp <= 20) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 w-full ${isPlayer ? '' : 'items-end'}`}>
      <div className={`flex items-baseline gap-2 ${isPlayer ? 'flex-row' : 'flex-row-reverse'}`}>
        ...
        <span className={`text-xs font-mono font-bold ${labelColor} px-2 py-0.5 rounded-md bg-white dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80`}>
          {clampedHp}/100 HP
        </span>
      </div>
      <div className="w-full h-3.5 bg-slate-200 dark:bg-slate-950/90 rounded-full border border-slate-300 dark:border-slate-800/80 p-0.5 overflow-hidden">
        <div
